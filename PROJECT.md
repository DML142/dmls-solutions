...
- **Navigation (decided)**: the global DOM navbar **slides up and hides** when the About section is active, then **reappears in a dark theme** once the user reaches Skills (hides again / reverts to light on scrolling back). In-section navigation is a **menu bar rendered inside the CRT scene** (HOME / ABOUT / SKILLS / CONTACT) — hover fills each button green left-to-right with black text, click scrolls to the section.
...
- **Scroll showcase (built)**: `Skills.tsx` is a tall (`h-[300vh]`, tunable) section with an inner `position: sticky` stage (`SkillsScene` R3F canvas + `SkillsGlow` background layer + `SkillsTextCard`), avoiding GSAP `ScrollTrigger` `pin: true` per the About section's seam-avoidance precedent. A single scrub `ScrollTrigger` (`useSkillsScrollProgress.ts`) drives a pure phase-mapping function (`lib/skillsPhases.ts`) that maps overall progress to which of the three logos is active and its enter→hold→exit sub-progress — fully reversible, no independent timers. Each logo slides+spins in from a fixed direction (Next.js/Other: left→right; NestJS: right→left), holds centered, then continues in the same direction to hide. The exported glTF models needed a per-model static rotation correction (they export lying flat) and a `<Center>` wrap (drei) plus per-model display-scale multipliers (their baked export scales aren't consistent with each other). The centered text card (title + divider + description) only shows content during each logo's hold window and never animates itself. Background glow crossfades white ↔ NestJS red-pink (`#e0234e`) straddling each phase boundary via `gsap.utils.interpolate`.
- **Navbar reveal (built)**: the DOM navbar reappears (dark theme) via a second `ScrollTrigger` boundary at the existing `releasePoint` (`TRIGGER_POINT + LOCK_DISTANCE`) — see `useHeroScrollAnimation.ts` for the yPercent reveal and `Navbar.tsx` for its own scroll-driven `theme` state (light/dark, prop-overridable).
- **In-scene menu navigation (decided)**: the CRT menu's HOME/ABOUT/SKILLS/CONTACT buttons all scroll via GSAP's `ScrollToPlugin` (`gsap.to(window, { scrollTo: ... })`, 1s `power2.inOut`, `autoKill:true`) instead of the browser's native `scrollTo(behavior:"smooth")`. HOME → top; ABOUT → `TRIGGER_POINT + LOCK_DISTANCE/2` (guaranteed inside the pinned/static range, not "scroll to document bottom" which used to work only because About was the last section — now overshoots into Skills); SKILLS → `document.getElementById('skills-section')`; CONTACT → `document.getElementById('contact-section')`.
...
- **Global contact popup (built)**: `ContactPopup.tsx` exports a `ContactPopupProvider` (mounted in `layout.tsx`, wrapping both `<NavBar />` and `{children}` — the only common ancestor of the navbar and the About section's CRT menu) and a `useContactPopup()` hook. The popup is a deliberate pastiche of a classic Android "Holo" `AlertDialog` — dark charcoal body, cyan title/rule/links/button, barely-rounded corners — always rendered in this fixed skin regardless of the navbar's light/dark theme, portaled to `document.body` above everything (`z-[100]`). It shows the same three contact links as `ContactCard.tsx` (copy-pasted, not shared — two consumers doesn't justify extracting a module) plus a CLOSE button, and dismisses via CLOSE, a backdrop click, or Escape. Page scroll is locked while open by setting `overflow: hidden` on both `document.documentElement` and `document.body` — locking only `body` doesn't work here since `document.scrollingElement` resolves to `<html>`, not `<body>`, in this app's standards-mode document.
- **Only the WaveButton opens the popup (decided)**: the navbar's inline "Contact" nav link and the CRT in-scene menu's "CONTACT" button both scroll to `#contact-section` via `scrollToSection`, same as every other nav target. Only the navbar's "CONTACT NOW" wave button (`WaveButton`'s `onClick`, wired directly to `openContactPopup()` in `Navbar.tsx`) opens the popup — these are two deliberately different affordances (jump to the section vs. a quick-glance popup), not three redundant paths to the same action.
- Status: **in progress** (scene, card, navbar zone, and contact popup all built; visual tuning remains).
